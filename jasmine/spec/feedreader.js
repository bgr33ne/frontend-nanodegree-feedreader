...
$(function() {
    ...
    describe('The menu', function() {
        ...
        it('changes visibility when clicked', function() {
            ...
            $hiddenState = $('body').hasClass(
                'menu-hidden');
            //clicks menu to change state
            $menuIcon.trigger('click');
            //checks new state against original state
            $hiddenStateOneClick = $('body').hasClass(
                'menu-hidden');
            expect($hiddenState).not.toMatch(
                $hiddenStateOneClick);
            //clicks menu again to change state
            $menuIcon.trigger('click');
            //checks new state against previous state
            $hiddenStateTwoClick = $('body').hasClass(
                'menu-hidden');
            expect($hiddenStateTwoClick).not.toMatch(
                $hiddenStateOneClick);
        });
    });
    ...
    describe('New Feed Selection', function(done) {
        ...
        it('new feed loaded has content that changes', function(
            done) {
            expect($('.feed .entry h2').html()).not.toBe(
                title);
            expect($('h1.header-title').html()).not.toBe(
                header);
            done();
        });
        //restores orignal state
